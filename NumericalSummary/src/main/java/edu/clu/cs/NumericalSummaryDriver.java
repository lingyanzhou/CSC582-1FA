...
public class NumericalSummaryDriver {
	...
	static public void main(String[] args) throws IOException,
		...
		try {
			...
			columnName = line.getOptionValue("c");
			if (line.hasOption("h")) {
				printHelp();
				System.exit(0);
			}
	...
	private static Options getOptions() {
		...
		options.addOption(OptionBuilder.withLongOpt("output")
				.isRequired().hasArg().withDescription("output folder").create("o"));
		options.addOption(OptionBuilder.withLongOpt("columns")
				.isRequired().hasArg().withDescription("columns").create("c"));
		options.addOption(OptionBuilder.withLongOpt("input")
				.isRequired().hasArg().withDescription("input folder").create("i"));
